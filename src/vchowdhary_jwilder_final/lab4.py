...
def updatePosition():
	...
	p1.x = int((pose.pose.position.x / 0.2))
	p1.y = int((pose.pose.position.y/ 0.2))
...
def visit(lofp, pub):
	...
	for p in lofp:
		point = Point()
		point.x = (p.x)*oc.info.resolution + 0.2 + oc.info.origin.position.x
		point.y = (p.y)*oc.info.resolution + 0.2 + oc.info.origin.position.y
...
def findFrontiers():
	...
	for f in fs:
		...
		for p in f:
			point = Point()
			point.x = p[0]*0.2 + 0.075 + oc.info.origin.position.x
			point.y = p[1]*0.2 + 0.075 + oc.info.origin.position.y
			grid.cells.append(point)
