...
class Astar:
	...
	def calculate(self):
		...
		while (not self.frontier.empty()):
			...
			for n in self.getAdjacent(curr):
				...
				if ((not n in self.visited) and self.within(n) and (not self.isOccupied(n))):
					a = [i for i in path]
					a.append(n)
	...
	def within(self, n):
		return n.x <= self.oc.info.resolution * self.oc.info.width * 2 and n.x >= 0 and n.y >= 0 and n.y <= self.oc.info.resolution * self.oc.info.width * 2
